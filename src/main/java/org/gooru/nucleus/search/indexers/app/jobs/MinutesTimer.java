...
public class MinutesTimer {
  private static final Logger LOGGER = LoggerFactory.getLogger(MinutesTimer.class);
  ...
  public static MinutesTimer schedule(Runnable runnable, int dayOfMonth, int hourOfDay, int minutes) {
    return new MinutesTimer(runnable, dayOfMonth, hourOfDay, minutes);
  }

  private MinutesTimer(Runnable runnable, int day, int hour, int minutes) {
    this.task = runnable;
    this.dayOfMonth = day;
    this.hourOfDay = hour;
