...
public class IndexerVerticle extends AbstractVerticle {
	...
	public void start(Future<Void> voidFuture) throws Exception {
		vertx.executeBlocking(blockingFuture -> {
			...
			if (future.succeeded()) {
				LOGGER.debug("Successfully initialized Indexer Handler machinery");
				voidFuture.complete();
			} else {
				voidFuture.fail("Not able to initialize the Indexer Handler machinery properly");
			}
		});
	}
	...
	private void startApplication() {
		...
		try {
			...
			LOGGER.debug("Error initializing application", e);
