...
public class LearningMapsRepositoryImpl extends BaseIndexRepo implements LearningMapsRepository {
  ...
  @Override
  public void saveRQCALearningMaps(List<Map<String, Object>> array) {
    array.forEach(dat -> {
   ...
   DB db = getDefaultDataSourceDBConnection();
    try {
      openConnection(db);
      db.openTransaction();
        db.exec(LearningMaps.INSERT_QUERY, data.get(EntityAttributeConstants.ID), 
  ...
  @Override
  public void saveCULForLearningMaps(Map<String, Object> data) {

   DB db = getDefaultDataSourceDBConnection();
    try {
      openConnection(db);
      db.openTransaction();
        db.exec(LearningMaps.INSERT_CUL_QUERY, data.get(EntityAttributeConstants.ID), 
