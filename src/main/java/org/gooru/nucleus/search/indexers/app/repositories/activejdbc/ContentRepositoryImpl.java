	...
	public JsonObject getQuestionAndParentContentIds(String collectionId) {
		...
		if(contents.size() > 0){
			for(Map content : contents){
				if(content.get(Content.CONTENT_FORMAT) != null && content.get(Content.CONTENT_FORMAT).equals(Content.CONTENT_FORMAT_QUESTION) && content.get(EntityAttributeConstants.ID) != null){
