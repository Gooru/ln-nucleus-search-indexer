...
public class KafkaNucleusConsumer {

  private final static Logger LOG = LoggerFactory.getLogger(KafkaNucleusConsumer.class);
  ...
  public KafkaNucleusConsumer(ConsumerConnector connector) {
    KafkaNucleusConsumer.consumer = connector;
  }
  ...
  public void start(final String kafkaTopic) {
    final Map<String, List<KafkaStream<byte[], byte[]>>> messageStreams =
            KafkaNucleusConsumer.consumer.createMessageStreams(ImmutableMap.of(kafkaTopic, 1));
    ...
    while (stream.iterator().hasNext()) {
      try {
       ...
       // LOG.debug("key : " + key + " Index message :" + message);
        System.out.println("consume receiving event {}"+ message);
