...
public class EsIndexServiceImpl implements IndexService {
	...
	public void indexDocumentByFields(String id, String indexName, String typeName, Map<String, Object> fieldValues) throws Exception {
		try{
			...
			getClient().prepareUpdate(indexName, typeName, id).setScript(new Script(scriptQuery.toString(),ScriptType.INLINE,"groovy", paramsField)).execute().actionGet();
			
			//Update Statistics Index 
			getClient().prepareUpdate(IndexNameHolder.getIndexName(EsIndex.STATISTICS), IndexerConstants.TYPE_STATISTICS, id).setScript(new Script(scriptQuery.toString(),ScriptType.INLINE,"groovy", paramsField)).execute().actionGet();
		}
		catch(Exception e){
		...
		new IdIterator(idString) {
			...
			public void execute(String indexableId) throws Exception {
				try{
					...
					getClient().prepareIndex(indexName, typeName, indexableId).setSource(EsIndexSrcBuilder.get(typeName).buildSource(result)).execute().actionGet();
