...
public class EsIndexServiceImpl extends BaseIndexService implements IndexService {
  ...
  public void bulkIndexDocuments(JsonArray jsonArr, String contentType, String index) {
    try{
      ...
      LOGGER.debug("bulkIndexDocuments()-> content type : {} total size : {}", contentType,  jsonArr.size());
      while(true) {
        ...
        for (Object jsonObjStr : jsonArr) {
          ...
          if (data != null) {
            String id = data.getString(EntityAttributeConstants.ID);
            ...
            // Get statistics and extracted text data from backup index
            setResourceStasInfoData(data, id, contentType);
            IndexRequest request = new IndexRequest(index, getIndexTypeByType(contentType), id); 
      ...
      if (response.getStatusLine().getStatusCode() == HttpStatus.SC_OK) {
        ...
        items.forEach(responseItem -> {
          ...
          if (responseJson.getBoolean("errors")) {
            ...
            DeleteService.instance().trackDeletes(updateError.getString("_id"), IndexerConstants.TYPE_RESOURCE);
