...
public class IndexScriptBuilder {
	...
	protected static final String INDEX_UPDATED = "if (ctx._source.containsKey(\"indexUpdatedDate\")) {ctx._source.indexUpdatedDate=indexUpdatedDate} else ({ctx._source.indexUpdatedDate=indexUpdatedDate})";
	...
	public static void buildScript(final String id, Map<String, Object> paramsField, StringBuffer scriptQuery, final Map<String, Object> fieldValueMap) {
			int i = 0;
			for(String field : fieldValueMap.keySet()){
				...
				if(value != null){
				    ...
				    createScript(paramKey, field, scriptQuery);
					++i;
				}
			}
	...
	private static void createScript(String paramKey, String key, StringBuffer scriptQuery){
		
		paramKey = key.replaceAll("\\.", "");
